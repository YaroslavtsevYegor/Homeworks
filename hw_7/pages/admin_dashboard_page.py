...
class DashboardPage(BasePage):
    navigation_tab = By.ID, 'button-menu'
    ...
    @allure.step("Choose 'Navigation'")
    def choose_navigation_tab(self, timeout=1):
        self.logger.info("%s: Clicking 'Navigation' tab" % (self.class_name))
        self.click(self.navigation_tab, timeout)

    @allure.step("Choose 'Catalog'")
    def choose_catalog_tab(self, timeout=1):
        self.logger.info("%s: Clicking 'Catalog' tab" % (self.class_name))
        self.click(self.catalog_tab, timeout)

    @allure.step("Choose 'Products'")
    def choose_products_tab(self, timeout=1):
        self.logger.info("%s: Clicking 'Products' button" % (self.class_name))
        self.click(self.products_tab, timeout)

    @allure.step("Add new {product_name}")
    def add_new_product(self, product_name, product_tag, product_model, product_keyword, timeout=1):
        self.logger.info("%s: Adding new item" % (self.class_name))
    ...
    def search_result(self, product_name, product_tag, product_model, product_keyword, timeout=1):
        ...
        self.logger.info("%s: Searching for item" % (self.class_name))
        self.input_value(self.product_name_filter_field, product_name, timeout)
        self.click(self.filter_button, timeout)
